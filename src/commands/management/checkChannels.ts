...
export async function checkChannels(
	...
	const deletedChannels = channels.filter(
		(channel) => !question.guild.channels.cache.has(channel.id),
	);
	...
	for (const deletedChannel of sortedChannels) {
		...
		if (newChannel.type === ChannelType.GuildCategory) {
			for (const parentChannel of deletedChannels.filter(
				(channel) => channel.parent === deletedChannel.id,
			)) {
				parentChannel.parent = newChannel.id;
			}
			await ChannelModel.updateMany(
				{ parent: deletedChannel.id },
				{ parent: newChannel.id },
			);

			const parentChannels = channels.filter(
				(channel) => channel.parent === deletedChannel.id,
			);
