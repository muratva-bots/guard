    ...
    execute: async ({ client, message, args }) => {
        ...
        if (["ekle", "add"].some(a => args[0] === a)) {
            ...
            const collectedOne = await question.awaitMessageComponent({
                filter: (i: MentionableSelectMenuInteraction) => i.user.id === message.author.id && i.isMentionableSelectMenu(),
                time: 1000 * 60 * 5,
            ...
            if (collectedOne) {
                ...
                const typeRow = new ActionRowBuilder<StringSelectMenuBuilder>({
                    components: [
                        new StringSelectMenuBuilder({
                            custom_id: "type",
                            max_values: 25,
                            options: Object.keys(titles).map((key) => ({ label: titles[key].name, value: key, description: titles[key].description }))
                        })
                    ],
                ...
                const collectedTwo = await question.awaitMessageComponent({
                    filter: (i: StringSelectMenuInteraction) => i.user.id === message.author.id && i.isStringSelectMenu(),
                    time: 1000 * 60 * 5,
                ...
                if (collectedTwo) {
                    ...
                    const safes = Array.from(client.safes).map(s => ({ id: s[0], allow: s[1] }));
            ...
            message.channel.send({
                embeds: [
                    embed.setDescription([
                        ...
                        client.safes.map((v, k) => `${client.users.cache.get(k) ? userMention(k) : roleMention(k)}: ${v.map(p => titles[p].name).join(", ")}`).join("\n")
                    ].join("\n"))
                ]
            });
        }
