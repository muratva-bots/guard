...
const Limit: Guard.ICommand = {
	...
	execute: async ({ client, message, guildData }) => {
							...
							[
								...
								limits
									.map(
										(l) =>
											`→ ${l.name}: ${ms(
												guildData.settings.guard[
													l.time
												] ||
													client.config.DEFAULTS.LIMIT
														.TIME,
											)} süre içinde ${
												guildData.settings.guard[
													l.count
		...
		const filter = (i: Interaction) =>
			...
			(i.isModalSubmit() || i.isStringSelectMenu());
		...
		collector.on("collect", async (i: Interaction) => {
			...
			if (i.isModalSubmit()) {
				...
				await GuildModel.updateOne(
					...
					{
						$set: {
							[`settings.guard.${limit.time}`]:
								guildData.settings.guard[limit.time],
							[`settings.guard.${limit.count}`]:
								guildData.settings.guard[limit.count],
						},
					},
					{ upsert: true },
				);
			...
			question.edit({
				embeds: [
					embed.setDescription(
						reason
							? reason
							: "Menünün süresi dolduğu için menü kapatıldı.",
					),
				],
				components: [],
			});
