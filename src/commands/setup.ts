...
const Setup: Guard.ICommand = {
	...
	execute: async ({ client, message, guildData }) => {
			...
			components: [
				new StringSelectMenuBuilder({
					...
					options: muscles.map((m) => ({
						...
						value: m.value,
						emoji: {
							id: guildData.settings.guard[m.value]
								? "1118846618259693638"
								: "1118834136858243112",
						},
					})),
			...
			embeds: [
				embed.setDescription(
					[
						...
						`${inlineCode(
							"𝓲",
						)} Aşağıda bulunan menüden korumaları açabilir veya kapatabilirsin.\n`,
						codeBlock(
							"yaml",
							[
		...
		collector.on("collect", async (i: StringSelectMenuInteraction) => {
			...
			guildData.settings.guard[muscle.value] =
				!guildData.settings.guard[muscle.value];
			await GuildModel.updateOne(
				{ id: message.guildId },
				{
					$set: {
						[`settings.guard.${muscle.value}`]:
							guildData.settings.guard[muscle.value],
					},
				},
				{ upsert: true },
						...
						[
							...
							codeBlock(
								...
								[
									`# ${
										message.guild.name
									} Sunucusunun Koruma Sistemi (Sistem Durumu: ${
										muscles.every(
											(m) =>
												!guildData.settings.guard[
													m.value
