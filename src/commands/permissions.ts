...
const Permissions: Guard.ICommand = {
    ...
    execute: async ({ client, message, args }) => {
        ...
        if (operation === "aç") {
            ...
            const dangerRoles = message.guild.roles.cache.filter((role) =>
                client.utils.dangerPerms.some((perm) => role.permissions.has(perm)) && !role.managed
            );
        ...
        message.channel.send({ content: `Bütün yetkiler ${bold(operation === "aç" ? "açıldı." : "kapatıldı.")}` })
    },
};
