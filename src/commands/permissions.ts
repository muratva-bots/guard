	...
	execute: async ({ client, message, args }) => {
		...
		if (!operation || !["aç", "kapat"].some((arg) => operation === arg)) {
			message.channel.send({
				content: `Lütfen geçerli bir argüman belirt! (${inlineCode(
					"aç",
				)} veya ${inlineCode("kapat")})`,
			});
			return;
		}

		if (client.utils.closingPermissions)
			client.utils.closingPermissions = false;

		const data =
			(await GuildModel.findOne({ id: message.guildId })) ||
			new GuildModel({ id: message.guildId });
		if (operation === "aç") {
			...
			const dangerRoles = message.guild.roles.cache.filter(
				(role) =>
					client.utils.dangerPerms.some((perm) =>
		...
		message.channel.send({
			content: `Bütün yetkiler ${bold(
				operation === "aç" ? "açıldı." : "kapatıldı.",
			)}`,
		});
	},
};
