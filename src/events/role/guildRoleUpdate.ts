...
const GuildRoleUpdate: Guard.IEvent = {
    ...
    execute: async (client, [oldRole, newRole]: Guard.ArgsOf<Events.GuildRoleUpdate>) => {
        try {
            ...
            const entry = await oldRole.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.RoleUpdate }).then((audit) => audit.entries.first());
            ...
            await oldRole.guild.members.ban(entry.executor.id, { reason: "Koruma!" });
            ...
            if (oldRole.guild.publicUpdatesChannel) {
                ...
                oldRole.guild.publicUpdatesChannel.send(`@everyone ${entry.executor} adlı kullanıcı ${roleName} adlı rolü ${action} ve yasaklandı.`);
            }
        } catch (error) {
