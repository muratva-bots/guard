...
async function presenceGuard(client: Client, guild: Guild) {
    ...
    const offlineOrWebStaffs = [...guild.members.cache.values()]
        .filter((m) =>
            ...
            guild.publicUpdatesChannel.send({
                embeds: [
                    embed.setDescription(
                        [
                            `${m} (${inlineCode(m.id)}) adlı kullanıcı ${m.presence?.clientStatus.web
    ...
    const oldDatas = [...guild.members.cache.values()]
        .filter(m =>
            client.staffs.has(m.id) &&
            !(
                (guildData.web && m.presence?.clientStatus.web) ||
