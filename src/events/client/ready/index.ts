...
const Ready: Guard.IEvent = {
    ...
    execute: async (client) => {
      ...
      const guildEventEmitter = GuildModel.watch([{ $match: { "fullDocument.id": guild.id } }], { fullDocument: "updateLookup" });
      guildEventEmitter.on("change", ({ fullDocument }: { fullDocument: IGuild }) => client.servers.set(guild.id, fullDocument));
    },
  };
