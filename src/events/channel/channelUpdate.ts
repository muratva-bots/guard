...
const ChannelUpdate: Guard.IEvent = {
	...
	execute: async (
		client,
		[oldChannel, newChannel]: Guard.ArgsOf<Events.ChannelUpdate>,
	) => {
		try {
			...
			const staffMember = oldChannel.guild.members.cache.get(
				entry.executorId,
			);
			const safe = [
				...[
			...
			if (limit) {
				if ((newChannel as GuildChannel).guild.publicUpdatesChannel) {
					...
					).guild.publicUpdatesChannel.send({ content });
				}
				return;
			}
