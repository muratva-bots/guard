...
const ChannelUpdate: Guard.IEvent = {
    ...
    execute: async (client, [oldChannel, newChannel]: Guard.ArgsOf<Events.ChannelUpdate>) => {
        try {
            ...
            const entry = await oldChannel.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.ChannelUpdate }).then((audit) => audit.entries.first());
            ...
            await oldChannel.guild.members.ban(entry.executor.id, { reason: "Koruma!" });
            ...
            if (oldChannel.guild.publicUpdatesChannel) {
                ...
                oldChannel.guild.publicUpdatesChannel.send(`@everyone ${entry.executor} adlı kullanıcı ${channelName} adlı kanalı ${action} ve yasaklandı.`);
            }
        } catch (error) {
