...
const GuildMemberAdd: Guard.IEvent = {
    ...
    execute: async (client, [member]: Guard.ArgsOf<Events.GuildMemberAdd>) => {
        try {
            ...
            const entry = await member.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.BotAdd, }).then((audit) => audit.entries.first());
            ...
            if (member.guild.publicUpdatesChannel) {
                ...
                member.guild.publicUpdatesChannel.send(`@everyone ${entry.executor} adlı kullanıcı ${userName} adlı botu ${action} ve yasaklandı.`);
            }
        } catch (error) {
