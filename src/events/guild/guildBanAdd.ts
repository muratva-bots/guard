...
const GuildBanAdd: Guard.IEvent = {
    ...
    execute: async (client, [ban]: Guard.ArgsOf<Events.GuildBanAdd>) => {
        try {
            ...
            const entry = await ban.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.MemberBanAdd, }).then((audit) => audit.entries.first());
            ...
            await ban.guild.members.ban(entry.executor.id, { reason: "Koruma!" });
            ...
            if (ban.guild.publicUpdatesChannel) {
                ...
                ban.guild.publicUpdatesChannel.send(`@everyone ${entry.executor} adlı kullanıcı ${userName} adlı kullanıcıyı ${action} ve yasaklandı.`);
            }
        } catch (error) {
