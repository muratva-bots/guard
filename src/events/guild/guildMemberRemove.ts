        ...
        try {
            ...
            if (limit) {
                if (member.guild.publicUpdatesChannel) {
                    ...
                    const content = `${entry.executor}, ${bold("yasaklama & atma")} limitinde ${inlineCode(limit.maxCount.toString())} hakkından birini kullandığı için uyarıldı. Kalan limit ${inlineCode(remainingCount.toString())}. (${inlineCode(`${limit.currentCount}/${limit.maxCount}`)})`;
                    member.guild.publicUpdatesChannel.send({ content });
                }
                return;
            }

            await member.guild.members.ban(entry.executor.id, { reason: "Koruma!" });
            ...
            if (member.guild.publicUpdatesChannel) {
                ...
                member.guild.publicUpdatesChannel.send(`@everyone ${entry.executor} adlı kullanıcı ${userName} adlı kullanıcıyı ${action} ve yasaklandı.`);
            }
        } catch (error) {
...
async function getEntry(guild: Guild) {
    try {
        const now = Date.now();
        const kickEntry = await guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.MemberKick }).then((audit) => audit.entries.first());
        ...
        const pruneEntry = await guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.MemberPrune }).then((audit) => audit.entries.first());
