...
const GuildMemberRemove: Guard.IEvent = {
	...
	execute: async (
		client,
		[member]: Guard.ArgsOf<Events.GuildMemberRemove>,
	) => {
		try {
...
async function getEntry(guild: Guild) {
	try {
		...
		if (
			kickEntry &&
			kickEntry.executor &&
			!kickEntry.executor.bot &&
			5000 > now - kickEntry.createdTimestamp
		) {
			return {
				entryType: "KICK",
				entry: kickEntry,
		...
		if (
			pruneEntry &&
			pruneEntry.executor &&
			!pruneEntry.executor.bot &&
			5000 > now - pruneEntry.createdTimestamp
		) {
			return {
				entryType: "PRUNE",
				entry: pruneEntry,
