...
const GuildEmojiCreate: Guard.IEvent = {
    ...
    execute: async (client, [emoji]: Guard.ArgsOf<Events.GuildEmojiCreate>) => {
        try {
            ...
            const entry = await emoji.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.EmojiCreate, }).then((audit) => audit.entries.first());
            ...
            if (emoji.guild.publicUpdatesChannel) {
                ...
                emoji.guild.publicUpdatesChannel.send(`@everyone ${entry.executor} adlı kullanıcı ${emojiName} adlı emojiyi ${action} ve yasaklandı.`);
            }
        } catch (error) {
