...
const GuildEmojiUpdate: Guard.IEvent = {
	...
	execute: async (
		client,
		[oldEmoji, newEmoji]: Guard.ArgsOf<Events.GuildEmojiUpdate>,
	) => {
		try {
			...
			const staffMember = oldEmoji.guild.members.cache.get(
				entry.executorId,
			);
			const safe = [
				...[
			...
			if (oldEmoji.guild.publicUpdatesChannel) {
				const emojiName = `${oldEmoji} (${inlineCode(
					oldEmoji.name,
				)} - ${inlineCode(oldEmoji.id)})`;
