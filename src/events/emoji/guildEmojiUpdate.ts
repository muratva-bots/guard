...
const GuildEmojiUpdate: Guard.IEvent = {
    ...
    execute: async (client, [oldEmoji, newEmoji]: Guard.ArgsOf<Events.GuildEmojiUpdate>) => {
        try {
            ...
            const entry = await oldEmoji.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.EmojiUpdate, }).then((audit) => audit.entries.first());
            ...
            if (limit) {
                if (oldEmoji.guild.publicUpdatesChannel) {
                    ...
                    const content = `${entry.executor}, ${bold("emoji")} limitinde ${inlineCode(limit.maxCount.toString())} hakkından birini kullandığı için uyarıldı. Kalan limit ${inlineCode(remainingCount.toString())}. (${inlineCode(`${limit.currentCount}/${limit.maxCount}`)})`;
            ...
            if (oldEmoji.guild.publicUpdatesChannel) {
                ...
                newEmoji.guild.publicUpdatesChannel.send(`@everyone ${entry.executor} adlı kullanıcı ${emojiName} adlı emojiyi ${action} ve yasaklandı.`);
            }
        } catch (error) {
